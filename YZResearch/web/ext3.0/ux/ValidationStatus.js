...
/**
 * @class Ext.ux.ValidationStatus A {@link Ext.StatusBar} plugin that provides
...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
	...
	resetForm : function(f) {
		if (f) {
			if (this.form) {
				this.stopMonitoring();
				this.form = Ext.getCmp(f).getForm();
				this.startMonitoring();
				this.form.on('beforeaction', function(f, action) {
					if (action.type == 'submit') {
						// Ignore monitoring while submitting otherwise the
						// field validation
	...
	init : function(sb) {
		sb.on('render', function() {
			this.statusBar = sb;
			this.defaultText=sb.defaultText;
			this.monitor = true;
		...
		sb.on('afterlayout', function() {
			// Grab the statusEl after the first layout.
				sb.statusEl.getEl().on('click', this.onStatusClick, this, {
					buffer :200
				});
			}, this, {
				single :true
	...
	updateErrorList : function() {
		if (this.errors.getCount() > 0) {
			...
			this.errors.each( function(err) {
				msg += ('<li id="x-err-' + err.field.id + '"><a href="#">'
						+ err.msg + '</a></li>');
			}, this);
	...
	showErrors : function() {
		...
		this.form.getEl().on('click', this.hideErrors, this, {
			single :true
